...
WORKSPACE=WiaSwift.xcworkspace
...
test: clean clean_simulators
	set -x -o pipefail && xcodebuild test -workspace $(WORKSPACE) \
		-scheme WiaSwift_iOS -destination 'id=$(SIM_ID)' | bundle exec xcpretty -c

integration_test: clean clean_simulators
	set -x -o pipefail && xcodebuild test -workspace $(WORKSPACE) \
		-scheme WiaSwift_iOS -configuration "API_Coverage" \
...
lint:
	swiftlint
	bundle exec pod lib lint WiaSwift.podspec
